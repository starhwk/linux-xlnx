 ...
 * PL.
 */
static void zynqmp_disp_av_buf_set_vid_clock_src(
		struct zynqmp_disp_av_buf *av_buf, bool from_ps)
{
 ...
 */
static void zynqmp_disp_av_buf_set_vid_timing_src(
		struct zynqmp_disp_av_buf *av_buf, bool internal)
{
	u32 reg = zynqmp_disp_read(av_buf->base, ZYNQMP_DISP_AV_BUF_CLK_SRC);
				     ...
				     enum zynqmp_disp_layer_mode mode)
{
	...
	zynqmp_disp_av_buf_disable_vid(&disp->av_buf, layer, mode);
...
static bool zynqmp_disp_layer_is_live(struct zynqmp_disp *disp)
{
	...
	for (i = 0; i < ZYNQMP_DISP_NUM_LAYERS; i++)
		if (disp->layers[i].mode == ZYNQMP_DISP_LAYER_LIVE)
			return true;
...
static inline struct zynqmp_disp_layer *bridge_to_layer(struct xlnx_bridge *bridge)
{
...
static int zynqmp_disp_bridge_set(struct xlnx_bridge *bridge,
...
static int zynqmp_disp_bridge_get_fmts(struct xlnx_bridge *bridge,
				      ...
				      u32 src_w, u32 src_h)
{
	...
	for (i = 0; i < drm_format_num_planes(fb->pixel_format); i++) {
		...
		layer->dma[i].sgl[0].size = width * cpp;
	...
	ret = zynqmp_disp_layer_set_fmt(layer->disp,  layer, fb->pixel_format);
	if (ret)
...
static void zynqmp_disp_plane_destroy(struct drm_plane *plane)
{
	...
	xlnx_bridge_unregister(layer->disp->drm, &layer->bridge);
...
static int zynqmp_disp_create_plane(struct zynqmp_disp *disp)
{
	...
	for (i = 0; i < ZYNQMP_DISP_NUM_LAYERS; i++) {
		...
		layer->bridge.set = &zynqmp_disp_bridge_set;
		layer->bridge.get_fmts = &zynqmp_disp_bridge_get_fmts;
		layer->bridge.of_node = layer->of_node;
		xlnx_bridge_register(disp->drm, &layer->bridge);
	}
...
static void zynqmp_disp_cancel_page_flip(struct xlnx_crtc *xlnx_crtc, struct drm_file *file)
{
	struct zynqmp_disp *disp = xlnx_crtc_to_disp(xlnx_crtc);
	...
	if (event && (event->base.file_priv == file)) {
		...
		complete_all(disp->event->base.completion);
		disp->event = NULL;
		kfree(&event->base);
		drm_crtc_vblank_put(crtc);
	}
...
static void zynqmp_disp_crtc_enable(struct drm_crtc *crtc)
{
	struct zynqmp_disp *disp = crtc_to_disp(crtc);
	int ret;
...
static void zynqmp_disp_crtc_disable(struct drm_crtc *crtc)
{
	struct zynqmp_disp *disp = crtc_to_disp(crtc);
...
{
	/* TODO: check disabling in atomic helper */
	/* Don't reply on vblank when disabling crtc */
...
static struct drm_crtc_helper_funcs zynqmp_disp_crtc_helper_funcs = {
	.enable		= zynqmp_disp_crtc_enable,
	.disable	= zynqmp_disp_crtc_disable,
				     ...
				     uint64_t val)
{
	...
	/* CRTC prop values are just stored here, and applied when CRTC gets enabled */
				     ...
				     uint64_t *val)
{
	...
	/* CRTC prop values are just stored here, and applied upon DPMS on */
	if (property == disp->color_prop)
		*val = disp->color;
	else if (property == disp->bg_c0_prop)
		*val = disp->bg_c0;
...
static void zynqmp_disp_create_crtc(struct zynqmp_disp *disp)
{
	...
	disp->xlnx_crtc.cancel_page_flip = &zynqmp_disp_cancel_page_flip;
...
int zynqmp_disp_probe(struct platform_device *pdev)
{
	...
	if (!IS_ERR(disp->_pl_pclk)) {
		...
		ret = zynqmp_disp_clk_enable_disable(disp->pclk, &disp->pclk_en);
		if (ret)
			disp->pclk = NULL;
	}
...
int zynqmp_disp_pm_suspend(struct device *dev)
{
	return 0;
}

int zynqmp_disp_pm_resume(struct device *dev)
{
	return 0;
}
